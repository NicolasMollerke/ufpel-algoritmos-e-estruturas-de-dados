...
int **fourSum( int *nums, int numsSize, int target,
    ...
    *returnSize = 0;
    ...
    int capacidade = 16;
    int **resul = malloc( capacidade * sizeof(int *) );
    int qTotal = 0;
    ...
    for ( int i = 0; i < numsSize - 3; i++ ) {
        for ( int j = i + 1; j < numsSize - 2; j++ ) {
            ...
            while ( esq < dir ) {
                long long somaFinal = (long long)nums[i] + nums[j] + nums[esq] + nums[dir];

                if ( somaFinal == target ) {

                    if ( qTotal == capacidade ) {
                        capacidade *= 2;
                        resul = realloc( resul, capacidade * sizeof(int *) );
                    }

                    resul[qTotal] = malloc( 4 * sizeof(int) );
                    resul[qTotal][0] = nums[i];
                    resul[qTotal][1] = nums[j];
                    resul[qTotal][2] = nums[esq];
                    resul[qTotal][3] = nums[dir];
                    qTotal++;
    ...
    qsort( resul, qTotal, sizeof(int *), CompararQuadruplas );

    *returnColumnSizes = malloc( qTotal * sizeof(int) );

    int q = 0;
    (*returnColumnSizes)[q] = 4;
    resul[q] = resul[0];
    q++;

    for ( int i = 1; i < qTotal; i++ ) {
        if ( resul[i][0] != resul[q-1][0] || resul[i][1] != resul[q-1][1] ||
             resul[i][2] != resul[q-1][2] || resul[i][3] != resul[q-1][3] ) {
