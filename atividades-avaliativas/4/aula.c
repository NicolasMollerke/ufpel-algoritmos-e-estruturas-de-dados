...
int **fourSum( int *nums, int numsSize, int target,
	...
	int **resul = malloc(200 * sizeof(int));

	returnColumnSizes = malloc(200 * sizeof(int));

	//4 ponteiros

	//vetores precisam ter no minimo 4 elementos para ter uma quadrupla
	if ( numsSize < 4 ){
		return 0;
	}

	int i = 0;
	int j = 1; //segundo elemento do vetor
	int t = numsSize-1; //ultimo elemento

	int p = t -1; //penultimo elemento
	int q = 0; //quadrupla atual


	qsort(nums, numsSize, sizeof(int), CompararInt); //ordena o vetor

	while (i < t){
		int soma1 = nums[i] + nums[t];
		int soma2 = nums[j] + nums[p];

		int somaFinal = soma1 + soma2;


		//se a soma dos 4 for menor do que o target deslocamos para a direita, pois o vetor esta ordenado
		//se for maior deslocamos para a esquerda visando diminuir a soma

		//todo valor a direita de um numero é maior ou igual a ele
		//todo valor a esquerda de um numero é menor ou igual a ele

		//se eu fizer target - o primeiro elemento, tenho que procurar outro elemento que

		if ( somaFinal < target ) {
			t++;
		} else if ( somaFinal > target ) {
			p--;
		}
		if ( somaFinal == 0 ){
			resul[q] = malloc(4 * sizeof(int*));
			resul[q][0] = nums[i];
			resul[q][1] = nums[j];
			resul[q][2] = nums[p];
			resul[q][3] = nums[t];
